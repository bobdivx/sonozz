...
export function buildDistroKidPackage({ keys, artist, track, cover, lyrics }) {
  ...
  const form = {
    ...
    instrumental: "No",
    primaryGenre: genre,
    recordLabel: keys?.distrokidLabel?.trim() || artist?.recordLabel?.trim() || artistName,
    producer: resolveProducerName(keys, {
      writerLegalName: keys?.distrokidLegalName,
      artistName,
    }),
    copyrightYear: String(year),
    copyrightOwner: `© ${year} ${keys?.distrokidLabel?.trim() || artist?.recordLabel?.trim() || artistName}`,
    phonogramYear: String(year),
    phonogramOwner: `℗ ${year} ${keys?.distrokidLabel?.trim() || artist?.recordLabel?.trim() || artistName}`,
    releaseDate,
